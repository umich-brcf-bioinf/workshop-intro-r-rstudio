...
The `BiocManager::install()` function can install CRAN, Bioconductor, and GitHub packages. Let's install a couple of useful packages. One, `janitor` helps to clean data tables and format column names in consistent ways. The other `gt` is a helpful package for formatting tables for reports. We'll see that in action a bit later.

```{verbatim, BiocManager_do_not_run, eval=FALSE, include=TRUE, class.source="doNotRun"}
# Install the janitor and gt packages
BiocManager::install(c('janitor', 'gt'))
```
...
We introduced the idea of a package, how to install them, and how to load them. We discussed different package repositories and their specialities.
