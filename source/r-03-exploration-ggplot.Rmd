...
```{r, 'plot2'}
# -------------------------------------------------------------------------
# Add a mapping: x-axis is gdpPercap
...
```{r, 'plot3'}
# -------------------------------------------------------------------------
# Add a "nice" label for the x-axis
...
```{r, 'plot5'}
# -------------------------------------------------------------------------
# Add a mapping: y-axis is lifeExp
# Add a nice label for the y-axis
# Add a geometry: geom_point() is a scatterplot
...
# -------------------------------------------------------------------------
# Add a mapping: color by continent
...
```{r, 'plot_x_log_scale'}
# -------------------------------------------------------------------------
# Change the scale of the x axis to log10
...
# -------------------------------------------------------------------------
# Change the colors assigned to each continent
...
```{r, 'brewer_pal_info2'}
# -------------------------------------------------------------------------
# Show attributes of the built-in color scales
...
```{r, 'plot_per_million'}
# -------------------------------------------------------------------------
# Add a mapping: size is "population in millions"
# Add nice labels for color and size mappings in the legend
...
```{r, 'head'}
# -------------------------------------------------------------------------
# Look at the first and last few rows of the dataset
...
```{r, 'full_plot_point'}
# -------------------------------------------------------------------------
# Plot the full dataset in a similar way to the 1997 only data
...
# -------------------------------------------------------------------------
# Calculate global mean life expectancy per year in the data
...
# -------------------------------------------------------------------------
# Calculate mean life expectancy per year per continent in the data
...
```{r, 'view'}
# -------------------------------------------------------------------------
# Calculate mean life expectancy per year per continent, and arrange by continent
...
# -------------------------------------------------------------------------
# Use View() to see a large table in a more convenient display format
...
```{r, 'data.frame'}
# -------------------------------------------------------------------------
# Coerce a tibble into a data.frame to see more of it in the Console

data.frame(life_exp_by_year_continent)
```
...
```{r, 'plot_lifeexp_line'}
# -------------------------------------------------------------------------
# Plot the mean life expectancy over the years, per continent as a line graph
...
# -------------------------------------------------------------------------
# First attempt at plotting life expectancy for all countries over time
...
# -------------------------------------------------------------------------
# Map the group to the country and try again
...
```{r, 'plot_box'}
# -------------------------------------------------------------------------
# For 1997 data only, make a boxplot of life expectancy across the continents
...
```{r, 'factor1'}
# -------------------------------------------------------------------------
# Add a new continent factor with mutate, where the order is specified
...
```{r, 'factor_preview'}
# -------------------------------------------------------------------------
# Preview the table with the additional column
...
```{r, 'head_continent'}
# -------------------------------------------------------------------------
# Look at the first few elements of the old and new column continent column
...
```{r, 'factor_boxplot'}
# -------------------------------------------------------------------------
# Use the new factor column in a boxplot to see change in continent ordering
...
```{r, 'plot_box_layer'}
# -------------------------------------------------------------------------
# Add a layer on top of the boxplot to show individual data composing each box
...
```{r, 'plot_box_jitter'}
# -------------------------------------------------------------------------
# Use geom_jitter() to prevent points from stacking
...
# -------------------------------------------------------------------------
# Adjust the width of the jitter
...
# -------------------------------------------------------------------------
# Experiment with reversing the order of the layers
...
# -------------------------------------------------------------------------
# Map point size to population within the jitter geometry
# NOTE: This applies the mapping only to the jitter layer
...
```{r, 'plot_box_color1'}
# -------------------------------------------------------------------------
# Change the color of the boxplot outline
# NOTE: The assignment is not mapped to the data
...
```{r, 'plot_box_color2'}
# -------------------------------------------------------------------------
# Use distinct colors in distinct geometries
...
```{r, 'plot_box_color4'}
# -------------------------------------------------------------------------
# Change the fill color of the boxes, mapping to the data
...
# -------------------------------------------------------------------------
# Change the bin size of the histogram
...
# -------------------------------------------------------------------------
# Create a facet plot of GDP per capita versus life expectancy across continents
...
# -------------------------------------------------------------------------
# Force the facets to display as stacked rows
# NOTE: This can make plots more comparable
...
```{r, 'ggsave_help', eval = FALSE}
# -------------------------------------------------------------------------
# Display the manual for ggsave()
...
```{r, 'change_theme'}
# -------------------------------------------------------------------------
# Use a built-in theme to change many attributes of a plot at once
...
```{r, 'change_font_size'}
# -------------------------------------------------------------------------
# Scale the size of all text in a plot
...
```{r, 'rotate_labels'}
# -------------------------------------------------------------------------
# Rotate the x-axis labels 90 degrees counter-clockwise
...
```{r, 'rotate_labels2'}
# -------------------------------------------------------------------------
# Adjust the justification after rotating the x-axis labels
...
```{r, 'modify_gapminder'}
# -------------------------------------------------------------------------
# Load the ggrepel package in preparation for point labeling

library(ggrepel)

# -------------------------------------------------------------------------
# Mutate the gm97 data to include a label column
# NOTE: We only add the desired labels and leave all others as the empty string
...
```{r, 'check_label1'}
# -------------------------------------------------------------------------
# View the mutated data to make sure we did the right thing
...
```{r, 'check_label2'}
# -------------------------------------------------------------------------
# View only the pertinent parts of the mutated data to verify
...
```{r, 'plot_labels'}
# -------------------------------------------------------------------------
# Add a label mapping and a label geometry
...
```{r, 'plot_labels2'}
# -------------------------------------------------------------------------
# Prevent global inheritence of aesthetic mappings by specifying the mapping
# only within the relevant geometry
...
In this lesson we introduced the concepts of data, aesthetics, and geometries to visualize a data table as a variaty of plots. We learned about different geometries for different types of data, how to adjust visual characteristics like color, fill, size, etc. We learned how to stratify plots with faceting.
...
# Exercise
 ...
 ```
 Is this plot clear? Might it be improved by plotting each continent on its own? Let's start by trying to alter the above code to plot just the Asian countries.

 ```{r, 'ggplot_exercise_asia'}
 gm |> filter(continent == 'Asia') |> ggplot(aes(x = year, y = gdpPercap, color = continent, group = country)) +
     geom_line() +
     labs(
         title = 'GDP per Capita 1952 - 2007 in Asia',
         x = 'Year', y = 'GDP per Capita') +
     theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
 ```
Instead of plotting each continent individually, we could try faceting the plot. Try adjusting the code from the first plot to generate the plot below.

 ```{r, 'ggplot_exercise_facet', echo = FALSE}
 gm |> ggplot(aes(x = year, y = gdpPercap, color = continent, group = country)) +
 ...
 1. What is the Asian country with the highest GDP per capita in the period 1950 - 1970?
 2. What are the two countries that are outliers in the Americas?
 3. What are the two African countries with strong GDP per capita growth through 1980, followed by a drop?
...
<br/>
<hr/>
| [Previous lesson](r-05-help.Rmd) | [Top of this lesson](#top) | [Wrap up](r-06-quarto.html) |
| :--- | :----: | ---: |